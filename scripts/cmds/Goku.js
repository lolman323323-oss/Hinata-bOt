...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get 6 random Goku images." },
    category: "image",
    guide: { en: "/goku - Get 6 random Goku images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Goku"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `goku_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `🔥 Here are 6 random Goku images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Goku Img Error:", err.message);
