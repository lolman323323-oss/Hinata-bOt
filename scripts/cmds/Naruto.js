...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Get 6 random Naruto cute images from Google Images." },
    category: "naruto",
    guide: { en: "/naruto - Get 6 Naruto cute images" },
  },

  onStart: async function ({ api, event }) {
    try {
      const query = "Naruto Uzumaki"; // Fixed search query
      const count = 6;
      ...
      for (let i = 0; i < Math.min(count, shuffled.length); i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `naruto_${i + 1}.jpg`);
      ...
      await api.sendMessage(
        { body: `🦊 Here are 6 random Naruto cute images!`, attachment: attachments },
        event.threadID,
        event.messageID
      );
      ...
      console.error("Naruto Cute Img Error:", err.message);
